...
const g = {
    ...
    getGasPriceInUSD(n, o, t) {
      ...
      return c.bigNumber(n).times(r).toNumber();
    },
    getPriceImpact({
      sourceTokenAmount: n,
      ...
      const i = c.bigNumber(n).times(o),
        a = c.bigNumber(r).times(t);
      return i.minus(a).div(i).times(100).toNumber();
    },
    getMaxSlippage(n, o) {
      const t = c.bigNumber(n).div(100);
      return c.multiply(o, t).toNumber();
    },
    getProviderFee(n, o = 0.0085) {
      return c.bigNumber(n).times(o).toString();
    },
    isInsufficientNetworkTokenForGas(n, o) {
      const t = o || "0";
      return c.bigNumber(n).eq(0) ? !0 : c.bigNumber(c.bigNumber(t)).gt(n);
    },
    isInsufficientSourceTokenForSwap(n, o, t) {
      const r = t?.find((a) => a.address === o)?.quantity?.numeric;
      return c.bigNumber(r || "0").lt(n);
    },
  },
  U = 15e4,
  O = 6,
  l = {
    ...
    networkTokenSymbol: "",
    inputError: void 0,
    slippage: D.CONVERT_SLIPPAGE_TOLERANCE,
    tokens: void 0,
  ...
  S = {
    ...
    getParams() {
      const n = p.state.activeChain,
        o = E.getCaipAddress(n) ?? p.state.activeCaipAddress,
        t = h.getPlainAddress(o),
        r = W(),
        i = _.getConnectorId(p.state.activeChain);
      ...
      const a = !e.toToken?.address || !e.toToken?.decimals,
        u =
          ...
          !c.bigNumber(e.sourceTokenAmount).gt(0),
    ...
    setSourceToken(n) {
      ...
      (e.sourceToken = n), s.setTokenPrice(n.address, "sourceToken");
    },
    ...
    setToToken(n) {
      ...
      (e.toToken = n), s.setTokenPrice(n.address, "toToken");
    },
    setToTokenAmount(n) {
      e.toTokenAmount = n ? c.toFixed(n, O) : "";
    },
    async setTokenPrice(n, o) {
      let t = e.tokensPriceMap[n] || 0;
      t || ((e.loadingPrices = !0), (t = await s.getAddressPrice(n))),
        o === "sourceToken"
          ? (e.sourceTokenPriceInUSD = t)
          : o === "toToken" && (e.toTokenPriceInUSD = t),
      ...
      if (!e.initializing) {
        if (((e.initializing = !0), !e.initialized))
          try {
            ...
            (e.initialized = !1),
              T.showError("Failed to initialize swap"),
              k.goBack();
          }
        e.initializing = !1;
      }
    ...
    async fetchTokens() {
      ...
      o &&
        ...
        s.setSourceToken(o),
        s.setSourceTokenAmount("0"));
    },
    async getTokenList() {
      const n = p.state.activeCaipNetwork?.caipNetworkId;
      if (!(e.caipNetworkId === n && e.tokens))
        try {
          e.tokensLoading = !0;
          const o = await w.getTokenList(n);
          (e.tokens = o),
            (e.caipNetworkId = n),
            (e.popularTokens = o.sort((t, r) =>
              t.symbol < r.symbol ? -1 : t.symbol > r.symbol ? 1 : 0
            )),
            (e.suggestedTokens = o.filter(
              (t) => !!D.SWAP_SUGGESTED_TOKENS.includes(t.symbol)
            ));
        } catch {
    ...
    async getAddressPrice(n) {
      ...
      if (o) return o;
      const r = (await A.fetchTokenPrice({ addresses: [n] }))?.fungibles || [],
        a = [...(e.tokens || []), ...(e.myTokensWithBalance || [])]?.find(
          (f) => f.address === n
        )?.symbol,
        u =
          r.find((f) => f.symbol.toLowerCase() === a?.toLowerCase())?.price ||
          0,
        d = parseFloat(u.toString());
      return (e.tokensPriceMap[n] = d), d;
    },
    async getNetworkTokenPrice() {
      const { networkAddress: n } = s.getParams(),
        t = (
          await A.fetchTokenPrice({ addresses: [n] }).catch(
            () => (
              T.showError("Failed to fetch network token price"),
              { fungibles: [] }
            )
          )
        ).fungibles?.[0],
        r = t?.price.toString() || "0";
      (e.tokensPriceMap[n] = parseFloat(r)),
        (e.networkTokenSymbol = t?.symbol || ""),
        (e.networkPrice = r);
    },
    async getMyTokensWithBalance(n) {
      const o = await L.getMyTokensWithBalance(n),
        t = w.mapBalancesToSwapTokens(o);
    ...
    setBalances(n) {
      ...
      n.forEach((i) => {
        ...
        (e.myTokensWithBalance = n.filter((i) =>
          i.address.startsWith(t.caipNetworkId)
        )),
        (e.networkBalanceInUSD = r
          ? c.multiply(r.quantity.numeric, r.price).toString()
          : "0");
    },
    async getInitialGasPrice() {
      const n = await w.fetchGasPrice();
      if (!n) return { gasPrice: null, gasPriceInUSD: null };
      switch (p.state?.activeCaipNetwork?.chainNamespace) {
        case m.CHAIN.SOLANA:
          return (
        ...
        default:
          ...
          return (
            (e.gasFee = o),
            (e.gasPriceInUSD = i),
            { gasPrice: t, gasPriceInUSD: i }
          );
      }
    },
    async swapTokens() {
      const n = E.state.address,
        o = e.sourceToken,
        t = e.toToken,
        r = c.bigNumber(e.sourceTokenAmount).gt(0);
      if ((r || s.setToTokenAmount(""), !t || !o || e.loadingPrices || !r))
        return;
      e.loadingQuote = !0;
      const i = c
        ...
        .round(0);
      try {
        const a = await A.fetchSwapQuote({
          userAddress: n,
          from: o.address,
          to: t.address,
          gasPrice: e.gasFee,
    ...
    async getTransaction() {
      ...
      if (!(!n || !o || !t || !r || e.loadingQuote))
        try {
          ...
          const i = await w.fetchSwapAllowance({
            userAddress: n,
          ...
          return (
            i
              ? (a = await s.createSwapTransaction())
              : (a = await s.createAllowanceTransaction()),
    ...
    async createAllowanceTransaction() {
      ...
      if (!(!n || !t)) {
        ...
        try {
          const r = await A.generateApproveCalldata({
              from: o,
              to: t,
              userAddress: n,
            }),
            i = h.getPlainAddress(r.tx.from);
          if (!i)
            throw new Error(
    ...
    async createSwapTransaction() {
      ...
      try {
        const u = await A.generateSwapCalldata({
            userAddress: o,
            from: r.address,
            to: i.address,
            amount: a,
          ...
          d = r.address === n,
          f = BigInt(u.tx.eip155.gas),
          C = BigInt(u.tx.eip155.gasPrice),
          v = h.getPlainAddress(u.tx.to);
        if (!v)
          throw new Error(
        ...
        return (
          (e.gasPriceInUSD = g.getGasPriceInUSD(e.networkPrice, f, C)),
          (e.approvalTransaction = void 0),
          (e.swapTransaction = N),
          N
        );
        ...
        (e.transactionError = a?.displayMessage),
          ...
          I.sendEvent({
            ...
            properties: {
              ...
              isSmartAccount: y(m.CHAIN.EVM) === b.ACCOUNT_TYPES.SMART_ACCOUNT,
            },
          });
      }
    },
    async sendTransactionForSwap(n) {
      ...
      const i = `Swapping ${
        ...
        } to ${c.formatNumberToLocalString(t, 3)} ${e.toToken?.symbol}`,
        a = `Swapped ${e.sourceToken?.symbol} to ${c.formatNumberToLocalString(
          t,
          3
        )} ${e.toToken?.symbol}`;
      r
        ? k.pushTransactionStack({
            onSuccess() {
              k.replace("Account"), T.showLoading(i), S.resetState();
            },
          })
        : T.showLoading("Confirm transaction in your wallet");
      try {
        const u = [e.sourceToken?.address, e.toToken?.address].join(","),
    ...
    setTransactionDetails() {
      ...
      !n ||
        !o ||
        ((e.gasPriceInUSD = g.getGasPriceInUSD(
          e.networkPrice,
          BigInt(e.gasFee),
          BigInt(U)
        )),
